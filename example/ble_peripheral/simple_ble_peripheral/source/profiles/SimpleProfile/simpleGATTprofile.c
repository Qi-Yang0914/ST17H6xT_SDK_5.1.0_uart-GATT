...
static uint8 simpleGATTProfileChar1Props = GATT_PROP_WRITE_NO_RSP;
    ...
    if ( pAttr->type.len == ATT_BT_UUID_SIZE )
    {
		...
		switch ( uuid )
		{
			case SIMPLEGATTPROFILE_CHAR1_UUID:
			{
				...
				LOG("write data:");
				LOG_DUMP_BYTE(LC_App_Set_Param.app_write_data, len);
				osal_set_event(LC_Uart_TaskID, UART_EVT_TEST_SEND);
			}
			break;
